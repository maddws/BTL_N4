...
import { Edit2, Trash2, Share2, FileText, FilePen, FileImage, FileCheck, FileClock, Calendar, User, Building2, FileDown, Eye } from 'lucide-react-native';
...
export default function MedicalDocumentDetailsScreen() {
  ...
  const handleShare = async () => {
    ...
    try {
      await Share.share({
        message: `Tài liệu y tế: ${document.title}\nNgày: ${formatDate(document.date)}\nLoại: ${getDocumentTypeName(document.type)}${document.description ? `\nMô tả: ${document.description}` : ''}`,
      ...
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={styles.documentTypeContainer}>
            ...
            <Text style={styles.documentType}>{getDocumentTypeName(document.type)}</Text>
          </View>
