...
export default function VaccinationDetailsScreen() {
  ...
  const handleDelete = () => {
    Alert.alert(
      "Xác nhận xóa",
      "Bạn có chắc chắn muốn xóa lịch tiêm phòng này không?",
      [
        {
          text: "Hủy",
          style: "cancel"
  ...
  const handleToggleComplete = () => {
    if (vaccination) {
      updateVaccination(vaccination.id, { completed: !vaccination.completed });
    }
  };
    ...
    <SafeAreaView style={styles.container} edges={['right', 'left']}>
      ...
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={[
            styles.statusBadge, 
            vaccination.completed ? styles.completedBadge : 
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity 
            style={[styles.completeButton, vaccination.completed && styles.incompleteButton]} 
            onPress={handleToggleComplete}
          >
            {vaccination.completed ? (
              <>
                <X size={20} color={Colors.card} />
                <Text style={styles.completeButtonText}>Đánh dấu chưa hoàn thành</Text>
              </>
            ) : (
              <>
