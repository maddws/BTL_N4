    ...
    useEffect(() => {
        ...
        const loadWeightData = async () => {
            if (activePet) {
                ...
                const startDateDay = startDate.toISOString().split('-')[2];
                const endDate = new Date(filterDate.end);
                const endDateDay = endDate.toISOString().split('-')[2];
    ...
    useEffect(() => {
        const loadActivityData = async () => {
            ...
            while (startDate <= endDate) {
                ...
                // if (startDate.getDate() === new Date(filterDate.start).getDate()) {
                //     labelData.push(startDate + dayOfWeek);
